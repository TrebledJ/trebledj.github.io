// Copied from https://www.npmjs.com/package/rapid-automated-keyword-extraction.
// Ugh, everybody's doing it yuckily.
...
export async function buildStopWordRegex(stopWordList) {
    ...
    //     }
    // })
    let reg = stopWordList.map(w => `\\b${w}\\b`).join('|');
...
export default async function rake(text, options) {
    let stopWordPattern = await process(options)
    let sentenceList = splitSentences(text)
    let phraseList = generateCandidateKeywords(text, stopWordPattern, minCharLength, maxWordsLength)
