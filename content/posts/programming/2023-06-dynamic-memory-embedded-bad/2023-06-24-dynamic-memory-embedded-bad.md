...
This applies to embedded system memory as well. It’s difficult to afford dynamic things with few resources. With more powerful MCUs, it’s easier to justify dynamic things—heap, polymorphism. In the end, we should consider the available resources, design requirements, and use cases, and decide accordingly. Let me summarise again. *If you **need** flexibility and can afford it, use dynamic memory. If you can’t afford it, use static.*^[The distinction between *want* and *need* is small, but IMO important when programming for embedded systems. Sometimes, we may *want* to use the heap, but it's not needed. Other times, we may *need* the flexibility of the heap since the benefits outweigh the costs.]
