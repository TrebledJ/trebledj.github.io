---
title: AOC 2021 Day 24 – Reverse Engineering in AOC???
description: When CTFs meet Advent of Code.
updated: "2022-09-01"
tags: aoc python reverse programming writeup
related_tags: aoc haskell
thumbnail: /assets/img/posts/aoc/z3.jpeg
---
...
At first glance, I thought it was something similar to the [intcode problems](https://adventofcode.com/2019/day/2) from two years before. After a more thorough read, it turns out this is a completely different problem. It also reminds me of reverse engineering challenges from [capture-the-flag competitions](/tags/ctf).
...
* `mul a b` multiples register `a` by register/value `b`,
...
You may notice that there are no jump instructions, so the ALU program is quite straightforward... I mean *actually **straightforward***. We just need to run each instruction once until the end.

Given the ALU program, we need to find the minimum and maximum ALU input which will cause the program to terminate with a `0` in register `z`. The ALU input is constrained to **14-digit** integers, and **all digits are non-zero**.

(Part 1 of the challenge is to just find the maximum input. Part 2 is to find the minimum.)

### Solving
While in the midst of reading the challenge, I had already decided on the tool to use. [Z3](https://github.com/Z3Prover/z3)!

![](/assets/img/posts/aoc/z3.jpeg)

Z3 is a logic programming engine (theorem prover, actually) developed by Microsoft Research. It's pretty useful for working with symbols and constraints and more importantly, deriving possible solutions for said symbols. For example, say we want to know what numbers $$x$$ can be used with $$x^2 = x$$. Z3 can solve this for us:
...
Now if we were given actual binaries, we would be able to throw them into a decompiler and get a control flow graph. But since the ALU instructions here are contrived, I decided not to bother and go with the fallback way of analysis:
...
* The 15 other instructions are the same in each block.
* Between each block, only the value of `z` is passed. `w`, `x`, and `y` are all local.
...
Firstly, we read our file and gather our intelligence like a shepherd gathering their flock at night.
...
And that's pretty much it. On my laptop, it spits out both solutions within half a minute. I would've hoped for better, but eh, at least it's much faster than a brute force check.
