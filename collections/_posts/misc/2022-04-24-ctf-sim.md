...
* We submit a writeup. This `malloc`s a chunk and inserts a payload of `win_addr`.
  ...
  * Anything equal or less than `0x8` means our payload will be cut off early.
  * Anything above `0x18` will allocate a new chunk instead of reusing the previously deallocated chunk.
* We solve the same challenge. This triggers a call to a virtual function which double-derefs our payload, calling `win()` and giving us a shell. PROFIT!
